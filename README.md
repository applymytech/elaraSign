...
## 🔐 IMPORTANT: Secrets & Signing Keys

```
╔══════════════════════════════════════════════════════════════════════════════╗
║                    ⚠️  NO SECRETS ARE INCLUDED IN THIS REPO  ⚠️               ║
║                                                                              ║
║  Every deployment MUST generate its own:                                     ║
║    • P12 signing certificate (auto-generated by setup.sh)                    ║
║    • Master encryption key (auto-generated by setup.sh)                      ║
║    • All secrets uploaded to Google Secret Manager                           ║
║                                                                              ║
║  Run ./setup.sh - it handles everything automatically.                       ║
║  YOUR KEYS = YOUR IDENTITY. They're never committed to git.                  ║
╚══════════════════════════════════════════════════════════════════════════════╝
```

**Why?** elaraSign is a **sovereign** signing service. Your signatures trace back to YOUR organization with YOUR certificate. Using someone else's keys would defeat the entire purpose.

Run `./setup.sh` to generate everything you need.
...
**Requirements:** Node.js 24+, Google Cloud account, Git Bash (Windows)

### Quick Deploy

```bash
git clone https://github.com/yourorg/elaraSign.git
cd elaraSign
./setup.sh      # Complete setup (interactive)
./preflight.sh  # Verify ready
./deploy.sh     # Deploy preview
./promote.sh    # Go live
```

That's it. setup.sh handles everything: APIs, secrets, permissions, config.

### The Scripts

| Script | What It Does |
|--------|--------------|
| `setup.sh` | Complete first-time setup (APIs, secrets, config) |
| `preflight.sh` | Verify everything ready before deploy |
| `deploy.sh` | Deploy new version with 0% traffic |
| `promote.sh` | Route 100% traffic to new version |
