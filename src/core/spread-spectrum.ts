...
const EMBED_POSITIONS: [number, number][] = [
  [1, 2], [2, 1], [2, 2], [1, 3], [3, 1], [2, 3], [3, 2], [3, 3],
...
const REDUNDANCY = 8; // Repeat payload 8 times for error correction
...
function dct8x8(block: number[][]): number[][] {
  const result: number[][] = Array(8).fill(null).map(() => Array(8).fill(0));
...
export function embedSpreadSpectrum(
  ...
  for (let by = 0; by < blocksY; by++) {
    for (let bx = 0; bx < blocksX; bx++) {
      ...
      const block: number[][] = Array(8).fill(null).map(() => Array(8).fill(0));
