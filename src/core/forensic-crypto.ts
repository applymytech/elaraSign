 ...
 */
export function encryptAccountability(
 ...
 */
export function decryptAccountability(
  encrypted: Uint8Array,
  masterKey: string,
  salt = ''
): DecryptedAccountability {
      ...
      for (const platformCode of Object.values(PLATFORM_CODES)) {
        ...
        try {
          ...
          if (storedChecksum === calculatedChecksum) {
            ...
            const platform = Object.entries(PLATFORM_CODES)
              .find(([_, code]) => code === platformCodeVal)?.[0] || 'unknown';
...
export function getPlatformCode(generator: string): number {
  if (generator.includes('desktop')) return PLATFORM_CODES['elara.desktop'];
  if (generator.includes('cloud')) return PLATFORM_CODES['elara.cloud'];
  if (generator.includes('sign.web') || generator === 'elaraSign.web') return PLATFORM_CODES['elara.sign.web'];
  if (generator.includes('sign.api') || generator.includes('sign.cloud')) return PLATFORM_CODES['elara.sign.api'];
